...
class APIService  {
    ...
    func createClients(name:String, models:[[String : Any]], rfc:String, promterId:String,
                       completion: @escaping (Result<Bool,APIError>) -> Void) async throws {
        
        //declare parameter as a dictionary which contains string as key and value combination.
        let parameters = ["name":name, "userEmail":user.email, "rfc": rfc, "models": models,
                          "promoterId":promterId] as [String : Any]
        ...
        if (response as? HTTPURLResponse)?.statusCode == 200 {
            ...
            errorData = try JSONDecoder().decode(ErrorData.self, from: data)
            print(errorData ?? "Null data")
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
    ...
    func getPromoters(completion: @escaping (Result<Bool,APIError>) -> Void) async throws  {
        //create the url with NSURL
        guard  let url = URL(string: "https://mb-control.azurewebsites.net/promoter") else {
            print("Invalid URL")
            return
        ...
        if (response as? HTTPURLResponse)?.statusCode == 200 {
            promoters = try JSONDecoder().decode(PromoterModels.self, from: data)
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                completion(.success(true))
            }
        }
